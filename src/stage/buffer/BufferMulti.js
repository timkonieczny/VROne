 ...
 * @param {Object} gl
 * @param {VROne.Object3D} object3D
 * @param {VROne.LightContainer} lightContainer
 * @class
 * @constructor
 */
VROne.BufferMulti = function (gl, object3D, lightContainer) {
    if (!object3D instanceof VROne.Object3D) throw "Can't create buffers for " + object3D;

    var modelViewMatrix = new VROne.Matrix4();
    modelViewMatrix.transposeUniform = false;
    //VR 
    var leftMatrix = new VROne.Matrix4();
    var rightMatrix = new VROne.Matrix4();
    var leftNormalMatrix = new VROne.Matrix3();
    var rightNormalMatrix = new VROne.Matrix3();
    ...
    var uniformValues = {
        ...
        normalMatrix: new VROne.Matrix3(),
...
//        console.log("Loading Shader File: " + object3D.shader);
        shaderVert = VROne.XMLLoader.getSourceSynch(object3D.shader + ".vert");
        shaderFrag = VROne.XMLLoader.getSourceSynch(object3D.shader + ".frag");
        ...
        var shaderGenerator = new VROne.ShaderGenerator();
    ...
    if(gl.shaders[shaderID]){
        ...
        shader = new VROne.Shader(gl, shaderVert, shaderFrag);
        gl.shaders[shaderID] = shader;
    }

    //Create Buffer
    var drawMode = useWireframe ? gl.LINES : undefined;
    var buffer = new VROne.Buffer(gl, object3D, shader, attributes, uniforms, drawMode);
    ...
    var copyMatrix = new VROne.Matrix4();
    /**
     * Updates perspective Matrix
     * @param {VROne.Camera} camera
     */
    this.update = function (camera) {
        //Create ModelViewMatrix
        VROne.Matrix4.prototype.copyToMatrix(object3D.modelMatrix, modelViewMatrix);
        modelViewMatrix.apply(camera.getViewMatrix());
        
        VROne.Matrix4.prototype.copyToMatrix(modelViewMatrix, copyMatrix);
        copyMatrix.invert();
    ...
    this.updateVR = function(camera){
        ...
        VROne.Matrix4.prototype.copyToMatrix(object3D.modelMatrix, leftMatrix);
        leftMatrix.apply(camera.getViewMatrix("left"));

        VROne.Matrix4.prototype.copyToMatrix(leftMatrix, copyMatrix);
        copyMatrix.invert();
        copyMatrix.transpose();
        copyMatrix.getMatrix3(leftNormalMatrix);

        //Create ModelViewMatrix Right eye
        VROne.Matrix4.prototype.copyToMatrix(object3D.modelMatrix, rightMatrix);
        rightMatrix.apply(camera.getViewMatrix("right"));

        VROne.Matrix4.prototype.copyToMatrix(rightMatrix, copyMatrix);
        copyMatrix.invert();
        copyMatrix.transpose();
