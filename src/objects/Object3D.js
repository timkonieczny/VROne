 ...
 * @constructor
 */
VROne.Object3D = function () {
    var scope = this;

    //Position and Rotation of the object
    this.position = new VROne.Vector3();
    this.rotation = new VROne.Quaternion();
    var positionGlobal = new VROne.Vector3();
    var rotationGlobal = new VROne.Quaternion();
    var rotationMatrix = new VROne.Matrix4();

    this.scale = new VROne.Vector3(1, 1, 1);
    this.modelMatrix = new VROne.Matrix4();

    this.parent = null;
    this.parentPositionWeight = new VROne.Vector3(1, 1, 1);
    this.parentRotationWeight = new VROne.Vector3(1, 1, 1);
    ...
    //Movement
    this.gravity = null;
    this.velocity = new VROne.Vector3();
    this.collision = false;
    this.useBoundingSphere = false;
    this.collideWith = null;
    this.onCollision = null;
    var lastUpdateTime = VROne.Timer.prototype.getCurrentTimeMs();
    ...
    this.calcBoundingBox = function(){
        ...
        this.boundingBox = new VROne.Geometry.Box(a, b, c);
        this.boundingBox.position = positionGlobal;
        this.boundingBox.offset = new VROne.Vector3(xMin, yMin, zMin);
    ...
    this.calcBoundingSphere = function(){
        ...
        var centerPoint =  new VROne.Vector3((xMax+xMin)/2, (yMax+yMin)/2, (zMax+zMin)/2);
        ...
        for(i=0; i < this.vertices.length; i+=3){
            ...
            var r = VROne.Vector3.prototype.distance(centerPoint, new VROne.Vector3(x, y, z));
            if(r > radius)radius = r;
        }

        this.boundingSphere = new VROne.Geometry.Sphere(radius);
    ...
    this.update = function(){
        var currentTime = VROne.Timer.prototype.getCurrentTimeMs();
            ...
            if(this.collision && this.collideWith){
                for(var i=0; i < this.collideWith.length; i++){
                    ...
                    if(check.useBoundingSphere && this.useBoundingSphere){
                        collisionCheck = VROne.Geometry.prototype.sphereSphereCollision(this.boundingSphere, check.boundingSphere);
                    }
                    else if(check.useBoundingSphere){
                        collisionCheck = VROne.Geometry.prototype.boxSphereCollision(this.boundingBox, check.boundingSphere, true);
                    }
                    else if(this.useBoundingSphere){
                        collisionCheck = VROne.Geometry.prototype.boxSphereCollision(check.boundingBox, this.boundingSphere);
                    }
                    else{
                        collisionCheck = VROne.Geometry.prototype.boxBoxCollision(this.boundingBox, check.boundingBox);
                    }
    ...
    var poolQuat = new VROne.Quaternion();
    var calcGlobalPosition = function(){
        if(scope.parent){
            ...
            VROne.Quaternion.prototype.copyToQuaternion(rotationGlobal, poolQuat);
            ...
            VROne.Vector3.prototype.copyToVector(scope.position, positionGlobal);
            VROne.Quaternion.prototype.copyToQuaternion(scope.rotation, rotationGlobal);
